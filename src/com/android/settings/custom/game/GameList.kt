...
package com.android.settings.custom.game
...
class GameList: Fragment(R.layout.game_list_layout) {
    ...
    private fun getInitialCheckedList(): List<String> {
        return GameModeUtils.getPackageList()
    }
    ...
    private fun onListUpdate(packageName: String, isChecked: Boolean) {
        ...
        if (isChecked) {
            GameModeUtils.add(packageName, GameManager.GAME_MODE_CUSTOM)
        } else {
            GameModeUtils.remove(packageName)
        }
        try {
